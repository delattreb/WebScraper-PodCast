...
class FranceCulture:
    ...
    def scrapxml(self):
        logger = com_logger.Logger('France Culture')
        
        table = []
        for conf in self.config['URLFRANCECULTURE']:
            url = self.config['URLFRANCECULTURE'][str(conf)]
            ...
            if requests.get(url).status_code == 200:
                ...
                for item_list in soup.find_all("item"):
                    ...
                    if com_sqlite.select(a_name) != a_name:
                        ...
                        table.append(a_name)
                        mail = com_email.Mail()
                        mail.send_mail_gmail("France Culture: " + a_name, table)
                    break
